...
const addCityConnection = async (cityConnection) => {
  ...
  const city1 = await cityController.getCityByCityName(City1)
  console.log('Here22')
  console.log(city1)
  if (_.isNil(city1)) {
    console.log('Error')
  ...
  const city2 = await cityController.getCityByCityName(City2)
  if (_.isNil(city2.city)) {
    console.log('Error')
  ...
  await cityController.addNeighbour(City1, City2).catch((err) => {
    console.log(err)
  })

  //  controller to add neighbour
  await cityController.addNeighbour(City2, City1).catch((err) => {
    console.log(err)
  })
}
...
const deleteCityConnection = async (cityConnection) => {
  ...
  //  controller to add neighbour
  await cityController.deleteNeighbour(City1, City2).catch((err) => {
    console.log(err)
  })

  //  controller to add neighbour
  await cityController.deleteNeighbour(City2, City1).catch((err) => {
    console.log(err)
  })
}
