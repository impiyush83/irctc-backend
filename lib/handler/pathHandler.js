...
const getPaths = async (req, res) => {
  ...
  if (_.isNil(city1)) {
    return res.status(400).json(ErrorUtil.buildError(400, 'source does not exists'))
  }

  // check if city2 exists
  const city2 = await CityDao.getCityByCityName(destination)
  if (_.isNil(city2)) {
    return res.status(400).json(ErrorUtil.buildError(400, 'destination does not exists'))
  }
